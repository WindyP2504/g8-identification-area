...
namespace VTP_Induction
{
    public class Util
    {
        ...
        public const string SEQ_FILE_EXT = ".job";

        public const string MODEL_FILE_EXT = ".Mdl";

        public const string MODEL_NAME_SEPRATE = "__";

        public const string MODEL_NAME_ERROR = "ERROR";
        ...
        public static string ShortArrayToStrUTF8(short[] dBytes)
        {
            if (dBytes != null && dBytes.Length > 0)
            {
                System.Text.UTF8Encoding enc = new System.Text.UTF8Encoding();
                return enc.GetString(dBytes.Select(x => (byte)x).ToArray());
            }
            else
            {
                return string.Empty;
        ...
        public static bool StrToBin(string str, ref int iBin)
        {
            iBin = -1;
            try
            {
                iBin = Convert.ToInt32(str, 2);
                return true;
            }
            catch
            {
        ...
        // convert a number to a string with number Bits
        // example input byteIn = 234, lenghtData = 10;
        // Output 0011101010
        public static string byteToBitsString(int byteIn, int lenghtData)
        {
            var bitsString = new StringBuilder(lenghtData);
            for (int i = lenghtData - 1; i >= 0; i--)
            {
                int pow = (int)Math.Pow(2, i);
                bitsString.Append(Convert.ToString((byteIn / pow) % 2));
            }
        ...
        //ex: 0x06 0x0D-> "06 0D"
        public static string ByteArrayToStrHexa(byte[] dBytes)
        {
        ...
        public static byte[] ConvertNumberToByteArray(byte nNumber, int nLenght)
        {
            byte[] nNumberReturn = new byte[nLenght];
            if (nLenght > 8)
            {
                return nNumberReturn;
            }
            for (int i = 0; i < nLenght; i++)
            {
                nNumberReturn[i] = (byte)((nNumber >> (8 * i)) & 0xff);
            }
            return nNumberReturn;
        }

        public static byte[] ConvertNumberToByteArray(ushort nNumber, int nLenght)
        {
            byte[] nNumberReturn = new byte[nLenght];
            if (nLenght > 8)
            {
        ...
        public static byte[] ConvertNumberToByteArray(uint nNumber, int nLenght)
        {
            byte[] nNumberReturn = new byte[nLenght];
            if (nLenght > 8)
            {
        ...
        public static byte[] ConvertNumberToByteArray(int nNumber, int nLenght)
        {
            byte[] nNumberReturn = new byte[nLenght];
            if (nLenght > 8)
            {
        ...
        public static byte[] ConvertNumberToByteArray(ulong nNumber, int nLenght)
        {
            byte[] nNumberReturn = new byte[nLenght];
            if (nLenght > 8)
            {
        ...
        public static void ProgramDataSave(string fileName, Type zclass, object dataObj)
        {
            bool bEncrypt = false;
                        ...
                        bEncrypt = false;
#endif
            //Create our own namespace for the output
            ...
            try
            {
                if (bEncrypt)
                {
                    ...
                    mySerializer.Serialize(writer, dataObj, ns);
                    writer.Close();
                    ...
                    {
                        using (var writerBin = new BinaryWriter(stream, Encoding.UTF8, false))
                        {
                            writerBin.Write(Encrypt(text));
                        }
                    }
                }
                else
        ...
        public static object ProgramDataLoad(string filename, Type zclass, bool bEncrypt = true)
        {
            object ret = null;

            XmlSerializer serializer = new XmlSerializer(zclass);
            FileStream fs = null;
            string sFileTemp = Path.GetDirectoryName(filename) + "\\Temp";
            try
            {
                try
                {
                    using (var stream = File.Open(filename, FileMode.Open))
                    {
                        ...
                        {
                            string createText = reader.ReadString();
                            createText = Decrypt(createText);
                            File.WriteAllText(sFileTemp, createText);
                        }
                    }
                }
                catch
                ...
                if (bEncrypt)
                {
                    fs = new FileStream(sFileTemp, FileMode.Open);
                }
                else
                {
        ...
        static string passPhrase = "VIETTEL";            // can be any string
        static string saltValue = "sALtTirodin";       // can be any string
        ...
        public static bool Delay(uint dwCount, ref bool bBreak)
        {
            long time = 0;
            bool bRet = true;
        ...
        public static string EraseFileExt(string sFileName)
        {
            ...
            for (int i = sFileName.Length - 1; i >= 0; i--)
            {
                ...
                if (cChar == '.')
                {
                    return sFileName.Substring(0, i - 1);
                }
            }
        ...
        public static bool IsNumberString(string sSrc)
        {
            ...
            for (int i = 0; i < sSrc.Length; i++)
            {
                ...
                if (cOneChar == '-' || cOneChar == '+' || cOneChar == 'E' || cOneChar == 'e' || cOneChar == '.')
                {
                    continue;
                }
        ...
        public static string GetStartEndStrBaseExtractionString(string sSrc, char cStartChar, char cEndChar)
        {
            ...
            for (int i = 0; i < sSrc.Length; i++)
            {
                ...
                if (cOneChar == cStartChar && !bFindStartChar)
                {
                    if (cOneChar == cEndChar && bFindStartChar)
                    {
                        return sStr;
                    }
                }
        ...
        public static string GetStrByIdx(string[] ArrStr, int idx)
        {
            string tmpStr = "UNKNOW";
            if (ArrStr == null || idx < 0 || idx >= ArrStr.Length)
            {
                return tmpStr;
            }
            else
        ...
        public static void SetComboBox(ref ComboBox cb, string s)
        {
            int nTemp = cb.FindStringExact(s);
            if (nTemp > -1)
            {
                cb.SelectedIndex = nTemp;
            }
        }

        public static void SetComboBox(ref DevExpress.XtraEditors.ComboBoxEdit cb, string s)
        {
            List<string> values = new List<string>();
            foreach (var item in cb.Properties.Items)
            {
                values.Add(item.ToString());
            }
            int nTemp = values.FindIndex(x => x == s);
            if (nTemp > -1)
            {
                cb.SelectedIndex = nTemp;
            }
        }
        public static string ArrStringIdx(string[] Arr, int idx)
        {
            string Str = "UNKNOW";
        ...
        public static string GetComboxString(ComboBox cb)
        {
            if (cb.Items.Count <= 0)
            {
                return "";
        ...
        public static string GetComboxString(DevExpress.XtraEditors.ComboBoxEdit cb)
        {
            ...
            try
            {
                using (TextFieldParser csvReader = new TextFieldParser(csv_file_path))
                {
                    ...
                    if (colFields != null)
                    {
                        ...
                        {
                            DataColumn FieldNamecolumn = new DataColumn(column);
                            FieldNamecolumn.AllowDBNull = true;
                            csvData.Columns.Add(FieldNamecolumn);
                        }
        ...
        public static string RepalceDeli(string str)
        {
